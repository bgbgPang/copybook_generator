...
# 执行fc-list命令，获取字体列表
cmd = ['fc-list', ':lang=zh', 'family']
output = subprocess.check_output(cmd, encoding='utf-8')
# 解析字体列表，获取字体名称和文件名之间的映射关系
systemFonts = {}
lines = output.strip().split('\n')
try:
    for line in lines:
        font_names = line.split(',')
        font_name = font_names[0]
        font_names.pop(0)
        for full_name in font_names:
            systemFonts[full_name.strip('.').replace("\\", "")] = font_name.strip()

except IndexError:
    print("can't get systemFonts")
...
class MainFrame(MyFrame):
    ...
    def __init__(self, *args, **kwds):
        ...
        self.combo_box_font.AppendItems(list(systemFonts.keys()))
        ...
        self.combo_box_font.SetValue(list(systemFonts.keys())[0])
    ...
    def OnSelectInput(self, event):
        ...
        dlg = wx.FileDialog(self, message="选择文件", defaultDir=os.getcwd(),
                            defaultFile='', wildcard="文本文件 (*.txt)|*.txt", )
    ...
    def OnGenerate(self, event):
        ...
        try:
            ...
            cfg['font'] = systemFonts[self.combo_box_font.GetValue()]
            ...
            self.info('错误 %s' + e, wx.ICON_ERROR)
